  ...
  return (
    ...
    >
      {text.split("").map((letter, i) => (
        <span key={i} aria-hidden="true" className={CHAR_CLASS}>
          {letter === " " ? "\u00A0" : letter}
        </span>
      ))}
...
// Renders kinetic character spans inline (no wrapper tag) so they can be
// composed inside an existing element alongside other segments.
    ...
    <>
      {text.split("").map((letter, i) => (
        <span
          key={i}
          aria-hidden="true"
          className={`${CHAR_CLASS}${className ? ` ${className}` : ""}`}
        >
          {letter === " " ? "\u00A0" : letter}
        </span>
      ))}
